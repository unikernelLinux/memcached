 ...
 */
struct stats_state {
    uint64_t      idle_kicks;   /* idle connections killed, written by conn_timeout_thread */
    uint64_t      curr_items;
    uint64_t      curr_bytes;
    uint64_t      curr_conns;
...
typedef struct {
    ...
    void *proxy_gc_timer; // periodic GC pushing.
...
struct conn {
    ...
    mc_resp_bundle *open_bundle; /* partially-used resp bundle for this conn */
...
conn *conn_new(const int sfd, const enum conn_states init_state, const int read_buffer_size,
...
extern __thread LIBEVENT_THREAD *worker_me;
#ifdef PROXY
